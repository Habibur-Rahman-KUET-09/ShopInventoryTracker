...
void main() {
  ...
  setUp(() async {
    tempDir = Directory.systemTemp.createTempSync('dokan_hisab_due_provider_test');
  ...
  test('addDueByName creates a new customer when the name is unknown',
      () async {
  ...
  test('addDueByName adds to the existing customer found by name (case-insensitive)',
      () async {
    final provider = DueProvider(DueRepository());
    await provider.addCustomer(name: 'করিম', phone: '017xxxxxxx', initialDue: 200);
